...
namespace Cinemachine
{
    ...
    public struct AxisState
    {
        ...
        public AxisState(
            ...
            string name, bool invert, float tValue =0f, bool useT=false)
        {
            m_MinValue = minValue;
            m_MaxValue = maxValue;
            m_Wrap = wrap;
        ...
        public bool Update(float deltaTime)
        {
            ...
            else if (!string.IsNullOrEmpty(m_InputAxisName))
            {
                try {
                    if (useTouchInput)
                    {
                        //TouchInputValue = Mathf.Clamp(TouchInputValue,-1,1);
                        m_InputAxisValue = TouchInputValue;
                    }
                    else
                    {
