...
public class CinemachineCameraOffset : CinemachineExtension
{
    ...
    public Vector3 Rotm_Offset = Vector3.zero;
    /// <summary>
    ...
    {   
        ...
        {   
            ...
            if (Rot )
            {
                //var a = state.RawOrientation.eulerAngles;
                //a += this.RotAngle;
                //state.RawOrientation = Quaternion.Euler(a.x, a.y, a.z);

                //var dir = (state.CorrectedPosition - state.ReferenceLookAt).normalized;
                //var dis = (state.CorrectedPosition - state.ReferenceLookAt).magnitude;

                //state.RawPosition = state.ReferenceLookAt + state.RawOrientation * dir * dis;

                //
                Quaternion qt = Quaternion.AngleAxis(RotAngle.y * Mathf.Deg2Rad, Vector3.up);
                state.RawPosition = qt * (state.RawPosition - state.ReferenceLookAt) + state.ReferenceLookAt;
                state.RawOrientation = qt * state.RawOrientation;
