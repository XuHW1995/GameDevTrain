...
public class freeClimbSystem : externalControllerBehavior
{
	...
	bool updateRaycastDetectionState (Vector3 currentRaycastPosition, Vector3 currentRaycastDirection, float currentFixedUpdateDeltaTime)
	{
				...
				if (!surfaceFoundOnSide) {
					if (!Physics.Raycast (currentRaycastPosition, currentRaycastDirection, out temporalHit, 
						...
						if (Physics.Raycast (currentRaycastPosition, newRaycastDirection, out hit, raycastDistance * 3, raycastLayermask)) {
							...
							if (showGizmo) {
								Debug.DrawRay (currentRaycastPosition, newRaycastDirection * 20, Color.black, 5);
							}
						}
					}
				}
		...
		if (Physics.Raycast (currentRaycastPosition, currentRaycastDirection, out hit, currentRaycastDistance, raycastLayermask)) {
			...
			if (showGizmo) {
				Debug.DrawRay (currentRaycastPosition, currentRaycastDirection * currentRaycastDistance, Color.gray, 5);
			}
		} else {
			if (showGizmo) {
				Debug.DrawRay (currentRaycastPosition, currentRaycastDirection * currentRaycastDistance, Color.magenta, 5);
			}
		}
	...
	bool updateCollisionDetectionState (Vector3 currentRaycastDirection, float currentFixedUpdateDeltaTime)
	{
		...
		if (adjustingPlayerToSurfaceCollisionDetection) {
			...
			Physics.Raycast (mainRaycastPosition, currentRaycastDirection, out hit, 2, raycastLayermask);
			...
			if (Time.time > lastTimeClimbActive + minDelayToAdjustDetectionCollision && closeToSurfaceDetected) {
				...
				if (climbDetectionCollisionPositionOffset != Vector3.zero) {
					newPosition += playerTransform.right * climbDetectionCollisionPositionOffset.x * (-1);
					newPosition += playerTransform.up * climbDetectionCollisionPositionOffset.y * (-1);
					newPosition += playerTransform.forward * climbDetectionCollisionPositionOffset.z * (-1);
				}
			...
			if (climbDetectionCollisionPositionOffset != Vector3.zero) {
				climbPosition += playerTransform.right * climbDetectionCollisionPositionOffset.x;
				climbPosition += playerTransform.up * climbDetectionCollisionPositionOffset.y;
				climbPosition += playerTransform.forward * climbDetectionCollisionPositionOffset.z;
			}
		...
		Vector3 collisionDetectionDirection = -lastClimbNormal;
...
//		print (raycastDirection);
		...
		if (!Physics.Raycast (raycastPosition, raycastDirection, out upperHit, 2, raycastLayermask) &&
			...
			Vector3 newRaycastPosition = playerTransform.position + (playerTransform.up * 0.5f) - playerTransform.forward * 0.6f;
...
//			raycastDirection = Vector3.Cross (playerTransform.right, currentNormal);
			...
			while (!surfaceToLedgeDetected && numberOfLoops < 60) {
				if (Physics.Raycast (newRaycastPosition, raycastDirection, out newHit, 2.6f, raycastLayermask)) {
					previousHit = newHit;
					...
					if (Physics.Raycast (climbSurfaceTargetPosition + currentNormal * 0.2f, raycastDirection - playerTransform.up, out topSurfaceHit, 3.3f, raycastLayermask)) {
